...
class ChiselRiscV(pluginTemplate):
    __model__ = "ChiselRiscV"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.top_dir = os.path.join(config['PATH'] if 'PATH' in config else "")
    ...
    def initialise(self, suite, work_dir, archtest_env):
        ...
        self.compile_cmd ='riscv{1}-unknown-elf-gcc -march={0} -mabi=ilp32 -static -mcmodel=medany -fvisibility=hidden -nostdlib -nostartfiles -g\
            -T '+self.pluginpath+'/env/link.ld\
            -I '+self.pluginpath+'/env/\
            -I ' + archtest_env + ' {2} -o {3} {4};\
            riscv{1}-unknown-elf-objcopy {3} -O binary {3}.bin;\
            od {3}.bin -An -tx1 -w1 -v > {3}.hex;\
            riscv{1}-unknown-elf-objdump {3} -b elf32-littleriscv -D > {3}.dump'
    ...
    def runTests(self, testList):
      ...
      for testname in testList:
          ...
          test_file = os.path.join(test_dir, 'my')

          # name of the signature file as per requirement of RISCOF. RISCOF expects the signature to
          # be named as DUT-<dut-name>.signature. The below variable creates an absolute path of
          # signature file.
          sig_file = os.path.join(test_dir, self.name[:-1])
          ...
          # function
          cmd = self.compile_cmd.format(testentry['isa'].lower(), self.xlen, test, test_file, compile_macros)
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = "cd {0};sbt \"testOnly ChiselRiscV.tests.HexTest -- -DprogramFile={1}.hex -DwriteVcd=1\" 2>&1 | tee {2}.stdout; `grep '^[a-f0-9]\+$$' {2}.stdout > {2}.signature`".format(self.top_dir, test_file, sig_file)
          else:
            simcmd = 'echo "NO RUN"'
